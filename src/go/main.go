...
func loadEnhancedSecurityConfig(configPath string, ruleEngine *EnhancedRuleEngine) error {
	...
	if !config.Global.EnableFileEvents &&
		!config.Global.EnableNetworkEvents &&
		!config.Global.EnableProcessEvents &&
		!config.Global.EnablePermissionEvents &&
		!config.Global.EnableMemoryEvents &&
		config.Global.MinUIDFilter == 0 &&
		config.Global.MaxUIDFilter == 0 &&
		config.Global.MaxEventsPerSecond == 0 &&
		config.Global.RingBufferSize == 0 &&
		config.Global.AlertThrottleSeconds == 0 &&
		config.Global.MaxAlertHistory == 0 &&
		config.Global.LogLevel == "" {
		config.Global = EnhancedGlobalConfig{
	...
	if totalRules == 0 {
		return fmt.Errorf("未加载到任何规则")
	}
	if totalRules > 0 && !hasConditions {
		return fmt.Errorf("规则已加载但没有任何有效条件")
	}
...
func main() {
	...
	// 创建 Python 服务客户端
	pythonClient := NewPythonServiceClient("127.0.0.1", 9900)
	log.Println("[+] Python 服务客户端已初始化")

	// 检查 Python 服务是否可用
	go func() {
		time.Sleep(2 * time.Second) // 等待 Python 服务启动
		if !pythonClient.HealthCheck() {
			log.Println("[!] Python 服务未响应，AI 检测和报告导出功能可能不可用")
		} else {
			log.Println("[+] Python 服务连接正常")
			...
			go func() {
				...
				defer ticker.Stop()
				for range ticker.C {
					if err := pythonClient.UpdateBaselines(); err != nil {
						log.Printf("[!] 更新 Python AI 基线失败: %v", err)
					}
				}
			}()
		}
	...
	if !*monitorOnly {
		alertAPI = NewAlertAPI(alertManager, *webPort, storage, eventContext, nil)
		if authService != nil {
	...
	if os.Geteuid() == 0 && ruleEngine.GlobalConfig.MinUIDFilter > 0 {
		log.Printf("Warning: 当前为root运行，自动关闭UID最小值过滤以确保事件可见")
		ruleEngine.GlobalConfig.MinUIDFilter = 0
	}
	...
	if configMap != nil {
		// 设置事件类型开关
		configMap.Put(uint32(0), uint64(boolToUint64(bool(ruleEngine.GlobalConfig.EnableFileEvents))))
		configMap.Put(uint32(1), uint64(boolToUint64(bool(ruleEngine.GlobalConfig.EnableNetworkEvents))))
		configMap.Put(uint32(2), uint64(boolToUint64(bool(ruleEngine.GlobalConfig.EnableProcessEvents))))
		configMap.Put(uint32(3), uint64(boolToUint64(bool(ruleEngine.GlobalConfig.EnablePermissionEvents))))
		configMap.Put(uint32(4), uint64(boolToUint64(bool(ruleEngine.GlobalConfig.EnableMemoryEvents))))
