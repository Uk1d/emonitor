...
type Parser struct {
	fieldMapper engine.FieldMapper
}

func NewTraceeParser() *Parser {
	return &Parser{
		fieldMapper: engine.NewTraceeFieldMapper(),
	}
}

func (p *Parser) SetFieldMapper(mapper engine.FieldMapper) {
	p.fieldMapper = mapper
}

func (p *Parser) Parse(data []byte) (*engine.RuleSet, error) {
	...
	rs := engine.NewRuleSet("tracee-signatures", engine.SourceTracee)
...
func (p *Parser) convertSignature(sig TraceeSignature) (*engine.UnifiedRule, error) {
	name := sig.Name
	...
	ur := &engine.UnifiedRule{
		...
		Severity:    engine.MapSeverity(severity),
...
func (p *Parser) buildConditions(sig TraceeSignature) []engine.ConditionExpr {
	...
	if len(eventNames) > 0 {
		if len(eventNames) == 1 {
			conditions = append(conditions, &engine.ComparisonExpr{
				...
				Value:    engine.MapEventType(eventNames[0]),
			})
		} else {
			mappedEvents := make([]interface{}, len(eventNames))
			for i, e := range eventNames {
				mappedEvents[i] = engine.MapEventType(e)
			}
			conditions = append(conditions, &engine.ComparisonExpr{
				Field:    "event_type",
...
func (p *Parser) buildSelectorConditions(selector TraceeSelector) []engine.ConditionExpr {
	conditions := make([]engine.ConditionExpr, 0)
...
func (p *Parser) buildArgFilterCondition(filter TraceeArgFilter) engine.ConditionExpr {
	if filter.ArgName == "" {
		return nil
	}
