...
func (p *Parser) Parse(data []byte) (*engine.RuleSet, error) {
	...
	rs := engine.NewRuleSet("falco-rules", engine.SourceFalco)
...
func (p *Parser) parseRule(item map[interface{}]interface{}) (*engine.UnifiedRule, error) {
	...
	ur := &engine.UnifiedRule{
		...
		Severity:     engine.MapSeverity(priority),
